...
def main(config: Path = typer.Option(Path("config.yaml"))) -> None:
    ...
    gs = GridSpec(1, 6, figure=fig, width_ratios=[1.25, 0.28, 0.9, 0.9, 0.22, 1.15], wspace=0.5)  # columns 1 and 4 are spacers
    ...
    if cross:
        ax.axvline(cross, color="k", ls=":", lw=0.9); ax.text(cross - 0.8, 0.02, f"P(S3)=P(S1)\n~{cross:.0f} C", fontsize=7, ha="right", va="bottom")
    ...
    ax2.set_ylabel("emission max (nm), exp.", labelpad=4); ax2.set_ylim(332, 352)
    ...
    for i, T in enumerate((25, 45)):
        ...
        ax.set_title(f"({'bc'[i]}) WTMetaD FES, {T} C\n1 us walker, not converged", fontsize=9)
    ...
    ax.legend(fontsize=7, markerscale=3, loc="upper left"); ax.set_title("(d) GMM (n=4), indole micro-environment", fontsize=9.5)
